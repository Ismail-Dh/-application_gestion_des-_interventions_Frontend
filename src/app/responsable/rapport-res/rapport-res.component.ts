...
@Component({
  selector: 'app-rapport-res',
  standalone: true,
  imports: [  CommonModule, 
        FormsModule,
        MatIconModule, 
        MatButtonModule,HeaderRespComponent,
        ReactiveFormsModule,MatFormFieldModule,
          ...
          MatNativeDateModule,
          MatDialogModule],
  templateUrl: './rapport-res.component.html',
  styleUrl: './rapport-res.component.scss'
})
export class RapportResComponent implements OnInit {
  searchTerm: string = '';
...
chargerInterventionsCompletes(): void {
  this.interventionService.getAllInterventions().subscribe((interventions: Intervention[]) => {
    ...
    const requetes = interventions
      .filter(inter => inter.statut === 'TERMINEE')  // 🔥 Filtre ici
      .map(intervention =>
        forkJoin({
    ...
    forkJoin(requetes).subscribe((interventionsCompletes: InterventionComplet[]) => {
      console.log("✅ Interventions TERMINEES :", interventionsCompletes);
...
generatePDF(intervention: InterventionComplet): void {
    const doc: any = new jsPDF();


    // 🔹 Ajouter un logo (en haut à droite)
    // ⚠️ Mets ton logo en Base64 (PNG ou JPG)
    // Exemple : assets/logo.png → convertis en base64 OU charge via service
    const img = '/assets/ocp.png'; // chemin relatif si Angular assets
    doc.addImage(img, 'PNG', 160, 10, 40, 20); // (x, y, largeur, hauteur)
    ...
    // 🔹 Infos principales
    doc.setFontSize(12);
    doc.text(`Référence Intervention : ${intervention.id}`, 14, 40);
    doc.text(`Statut : ${this.getStatusLabel(intervention.statut)}`, 14, 50);
    doc.text(`Date début : ${this.formatDate(intervention.dateDebut)}`, 14, 60);
    doc.text(`Date fin : ${this.formatDate(intervention.dateFin)}`, 14, 70);

    // 🔹 Infos Demande
    autoTable(doc, {
      startY: 80,
      head: [['Titre', 'Description', 'Priorité', 'Niveau']],
      body: [[
        intervention.Demande.titre,
        intervention.Demande.description,
        this.getPriorityLabel(intervention.Demande.priorite),
        intervention.Demande.niveauIntervention || 'N/A'
      ]],
    });
    ...
    autoTable(doc, {
      ...
      body: [[
        ...
        intervention.Demandeur.prenom,
        intervention.Demandeur.email || 'N/A'
      ]],
    });
    ...
    autoTable(doc, {
      startY: doc.lastAutoTable.finalY + 10,
      head: [['Technicien', 'Commentaires']],
      body: [[
        `${intervention.Technicien.nom} ${intervention.Technicien.prenom}`,
        intervention.commentaires || 'Aucun'
      ]],
    });
    ...
    autoTable(doc, {
      startY: doc.lastAutoTable.finalY + 10,
      head: [['Équipement', 'Diagnostic', 'Solution']],
      body: [[
        intervention.Equipement.nom,
        intervention.diagnostics || 'N/A',
        intervention.solutionApporte || 'N/A'
      ]],
    });

    // 🔹 Signature / Validation
    doc.text(
      `Validée par le demandeur : ${intervention.valideParDemandeur ? 'Oui' : 'Non'}`,
      14,
